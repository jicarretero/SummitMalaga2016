...
ID_APACHE=$(nova boot --nic net-id=$NETUUID --image $IMAGE --key-name $KEY --security-groups $SECGROUP --flavor $FLAVOR --block-device-mapping vdb=$VOLUMEID --user-data InstallApacheThings $NAME | awk '/ id / {print $4}')
